...
public class PolicyDataSource implements Closeable {
    ...
    /**
     * Initializes the class PolicyDataSource. Tries to read the environment variables needed.
    ...
    public List<Policy.Builder> getPoliciesByValidityDateBetween(final Date from, final Date to) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            try(PreparedStatement preparedStatement = connection.prepareStatement(SQL_POLICY_VALIDITY_DATE_BETWEEN)) {
                ...
                LocalDate ldFrom = Instant.ofEpochMilli(from.getTime()).atZone(ZoneId.of("CET")).toLocalDate();
                LocalDate ldTo = Instant.ofEpochMilli(to.getTime()).atZone(ZoneId.of("CET")).toLocalDate();
